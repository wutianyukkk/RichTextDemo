...
ZSSEditor.refreshVisibleViewportSize = function(edit) {
    $(document.body).css('min-height', window.innerHeight + 'px');
    if(edit == 1){
        $('#zss_field_content').css('min-height', (window.innerHeight - $('#header_content').height - $('#footer_content').height) -  + 'px');
    }
};
...
// MARK: - Images  编辑内容下的图片
ZSSEditor.getImageCount = function() {
    //获取 图片节点总数
    // $("img#data-wpid")
    var length = $("#zss_field_content img").length;
    return length;
};

ZSSEditor.getContentLocalImagePaths = function() {
    //获取 含有属性为data-wpid 的img节点数组
    var nodes = new Array();
    // $("img#data-wpid")
    $("#zss_field_content img[local-ios]").each(function(){
                             nodes.push(this.src)
                             });
    return nodes.join(",");
};

ZSSEditor.replaceContentUrlPaths = function(urlString) {
    var nodes = $("#zss_field_content img[local-ios]");
    var urls = urlString.split(",");
    nodes.each(function(index,value){
               value.src = urls[index];
               value.removeAttribute('local-ios');
               });
    
}

ZSSEditor.removeContentUrlPaths = function(urlString) {
    var nodes = $("#zss_field_content img[local-ios]");
    var urls = urlString.split(",");
    nodes.each(function(index,value){
               if(!ZSSEditor.contains(urls,value.src)){
               value.parentNode.removedNode(value);
               }
               });
    
}

ZSSEditor.contains =function(arr, obj){
    var i = arr.length;
    while (i--) {
        if (arr[i] === obj) {
            return true;
...
// MARK: - Images  编辑底部下的图片（用于预览实现）
ZSSEditor.getFooterLocalImagePaths = function() {
    //获取 含有属性为data-wpid 的img节点数组
    var nodes = new Array();
    // $("img#data-wpid")
    $("#footer_content img[local-ios]").each(function(){
                                                nodes.push(this.src)
                                                });
    return nodes.join(",");
};

ZSSEditor.replaceFooterUrlPaths = function(urlString) {
    var nodes = $("#footer_content img[local-ios]");
    var urls = urlString.split(",");
    nodes.each(function(index,value){
               value.src = urls[index];
               value.removeAttribute('local-ios');
...
ZSSEditor.insertLocalImagePath = function(imageNodeIdentifier, localImageUrl, width, height) {
    
    if (window.getSelection) {
        sel = window.getSelection();
        if (sel.getRangeAt && sel.rangeCount) {
            var image = document.createElement("img");
            image.setAttribute('src', localImageUrl);
            image.setAttribute('class','alignnone size-full');
            image.setAttribute("local-ios", imageNodeIdentifier);
            image.alt = "";
...
ZSSEditor.markImageUploadDone = function(imageNodeIdentifier, image, mediaID) {
    ...
    if (imageNode.length > 0) {
        ...
        imageNode.attr("local-ios", mediaID);
    }
...
ZSSEditor.sendEnabledStyles = function(e) {
    ...
    if (focusedField && !focusedField.hasNoStyle) {
        ...
        if (typeof(e) != "undefined") {
            ...
            // The target element
            var t = $(ZSSEditor.getSelectedNode());
            ...
            try
            {
                ...
                if (textColor && textColor.length != 0) {
                    items.push('textColor:' + ZSSEditor.rgbToHex(textColor));
                }
...
ZSSEditor.rgbToHex =function(rgb){
    // rgb(x, y, z)
...
function ZSSField(wrappedObject,
    ...
    this.bodyPlaceholderColor = '#a0a0a0';
...
//设置字体大小
ZSSField.prototype.setFontSize = function(size) {
    this.wrappedObject.css('font-size', size+'px');
};

ZSSField.prototype.setLetterSpacing = function(size) {
    this.wrappedObject.css('letter-spacing', size+'px');
};

ZSSField.prototype.setLineHeight = function(size) {
    this.wrappedObject.css('line-height', size+'px');
};

//设置高度
ZSSField.prototype.setFieldHeight = function(fieldHeight) {
    this.wrappedObject.css('height', fieldHeight);
};
...
ZSSField.prototype.wrapCaretInParagraphIfNecessary = function()
{
    ...
    if (parentNodeShouldBeParagraph && closerParentNode.innerHTML.l) {
...
ZSSField.prototype.getInnerHTML = function() {
...
ZSSField.prototype.limitHTML = function(maxLength) {
    var node = this.wrappedObject.clone();
    var children = node.children();
    var html = "";
    for (var i = 0 ; i < children.length ; i++){
        var childrenNode = children[i];
        
        var subChildren = childrenNode.children;
        for(var j = 0 ; j < subChildren.length ; j++) {
            var subchildrenNode = subChildren[j];
            if(subchildrenNode.nodeName == "IMG" || subchildrenNode.nodeName == "img"){
                childrenNode.removeChild(subchildrenNode);
            }
        }
        
        var content = childrenNode.textContent;
        if(content.length < maxLength){
            html += childrenNode.outerHTML;
            maxLength -= content.length;
        } else {
            var content = content.substring(0,maxLength);
            childrenNode.textContent = content;
            html += childrenNode.outerHTML;
            break;
        }
    }
...
ZSSField.prototype.setMaxHeight = function() {
    if(!this.wrappedObject.hasClass('content-maxheight')){
        this.wrappedObject.addClass('content-maxheight');
    }
};

ZSSField.prototype.removeMaxHeight = function() {
    if(this.wrappedObject.hasClass('content-maxheight')){
        this.wrappedObject.removeClass('content-maxheight');
    }
};

ZSSField.prototype.contentHeight = function() {
